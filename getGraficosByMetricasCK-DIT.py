...
# -*- coding: utf-8 -*-
"""
Gráfico da evolução da métrica DIT ao longo das releases.
...
# ------------------------------------------------------------
# 2. Agregar DIT por release
# ------------------------------------------------------------
dit_stats = full_df.groupby('release_order')['dit'].agg(['mean', 'std', 'count'])
dit_stats['ci95'] = 1.96 * dit_stats['std'] / np.sqrt(dit_stats['count'])
...
ax.plot(x, y, marker='o', color='darkorange', linewidth=2, label='Média do DIT')
ax.fill_between(x, y - ci, y + ci, alpha=0.3, color='darkorange', label='IC 95%')

# Eixo X com nomes das releases
ax.set_xticks(x)
ax.set_xticklabels(release_names, rotation=45, ha='right', fontsize=8)
ax.set_ylabel('DIT médio')
ax.set_title('Evolução da métrica DIT (Profundidade da Árvore de Herança)')
...
output_file = 'ck_archive/plots/evolucao_dit.png'
