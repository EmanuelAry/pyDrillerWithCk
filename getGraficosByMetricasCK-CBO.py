...
# -*- coding: utf-8 -*-
"""
Gráfico da evolução da métrica CBO ao longo das releases.
...
# ------------------------------------------------------------
# 2. Agregar CBO por release
# ------------------------------------------------------------
cbo_stats = full_df.groupby('release_order')['cbo'].agg(['mean', 'std', 'count'])
cbo_stats['ci95'] = 1.96 * cbo_stats['std'] / np.sqrt(cbo_stats['count'])
...
ax.plot(x, y, marker='o', color='crimson', linewidth=2, label='Média do CBO')
ax.fill_between(x, y - ci, y + ci, alpha=0.3, color='crimson', label='IC 95%')

# Eixo X com nomes das releases
ax.set_xticks(x)
ax.set_xticklabels(release_names, rotation=45, ha='right', fontsize=8)
ax.set_ylabel('CBO médio')
ax.set_title('Evolução da métrica CBO (Acoplamento entre Objetos)')
...
output_file = 'ck_archive/plots/evolucao_cbo.png'
