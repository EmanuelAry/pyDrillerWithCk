...
# -*- coding: utf-8 -*-
"""
Gráfico da evolução da métrica LCOM1 ao longo das releases.
...
# ------------------------------------------------------------
# 2. Agregar LCOM1 (coluna 'lcom') por release
# ------------------------------------------------------------
lcom_stats = full_df.groupby('release_order')['lcom'].agg(['mean', 'std', 'count'])
lcom_stats['ci95'] = 1.96 * lcom_stats['std'] / np.sqrt(lcom_stats['count'])
...
ax.plot(x, y, marker='o', color='darkviolet', linewidth=2, label='Média do LCOM1')
ax.fill_between(x, y - ci, y + ci, alpha=0.3, color='darkviolet', label='IC 95%')

# Eixo X com nomes das releases
ax.set_xticks(x)
ax.set_xticklabels(release_names, rotation=45, ha='right', fontsize=8)
ax.set_ylabel('LCOM1 médio')
ax.set_title('Evolução da métrica LCOM1 (Falta de Coesão dos Métodos)')
...
output_file = 'ck_archive/plots/evolucao_lcom1.png'
